...
function semipiechart(titolGrafica, fillColor, percentatge, maxPercentatge, action, div_id) {
    percentatge = parseFloat(percentatge.replace(",", "."), 10);
...
function drawSemipiechart(
    ...
    var pathForeground = PintarArcColorSemipiechart(svg, arcLine, percentatge, maxPercentatge, fillColor);

    // Aplicar el filtro de sombra al arco coloreado para el efecto 3D
    pathForeground.attr("filter", "url(#shadow)");
    ...
    var arcTween = function (transition, newValue, oldValue) {
        transition.attrTween("d", function (d) {
            ...
            return function (t) {
                ...
                middleCount.text(Math.floor(interpolateCount(t) * 100) / 100 + '%');
    ...
    pathForeground.transition()
        .duration(3500)
        .call(arcTween, percentatge, oldValue);
}
...
function AfegirArcBaseSemipiechart(innerRadius, outerRadius) {
    return d3.arc() //definicio de les variables d'arc
        .innerRadius(innerRadius) //radi interior 
        .outerRadius(outerRadius) //radi exterior
        .startAngle(0) //angle inici
        .endAngle(Math.PI); //angle final
}

function AfegirArcColorSemipiechart(innerRadius, outerRadius) {
    return d3.arc()//arc amb el color pintat
...
function PintarArcBaseSemipiechart(svg, arc, emptyColor) {
    svg.append('path')//afegir arc al svg
        .attr("d", arc)
        .attr("transform", 'rotate(-90)')
        .attr('stroke-width', "5")
...
function AfegirSvgSemipiechart(div_id, width, height) {
    return d3.select(div_id) //creacio de grafica
        .append('svg')
        .attr('width', width)
        .attr('height', height)
        .style("position", "absolute")
...
function AfegirTextPercentatge(svg, textColor, innerRadius) {
    return svg.append('text') //percentatge amb animacio
        .datum(0)
        .text(function (d) { return d; })
        .attr("class", 'middleText')
        .attr('text-anchor', 'middle')
        .attr("dy", 10) //posicio respect les bases d'arc
        .style("fill", textColor) //color text
        .style('font-size', '' + (innerRadius / 2) + 'px');//mida text
}
